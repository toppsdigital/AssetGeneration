...
export async function POST(request: NextRequest) {
  try {
    ...
    if (client_method === 'put') {
      ...
      if (!putRes.ok) {
        ...
        return NextResponse.json(
          { error: 'Failed to get presigned S3 PUT URL', details: errorData }, 
          { status: 500 }
        );
      }
      ...
      if (upload) {
        return NextResponse.json({ uploadUrl: '/api/s3-upload', presignedUrl: url });
      }
    ...
    if (client_method === 'get') {
      ...
      if (!filename && !key) {
        return NextResponse.json({ error: 'Missing filename or key for GET request' }, { status: 400 });
      }
      ...
      if (!getRes.ok) {
        return NextResponse.json({ error: 'Failed to get presigned S3 GET URL' }, { status: 500 });
      }
    ...
    console.log('S3 Proxy - Request body:', JSON.stringify(body, null, 2));
